...
> [!TIP]  
> **If you want to contribute to open source projects and learn `Rust`, `gRPC`, `RUST API`, `tonic`, `diesel`, `sqlite`, `SurrealDB`, `tikv`, `Raft` protocol, `Mainline DHT`, `BitTorrent`, `QUIC`, `Dirct I/O`, `FUSE`, `Cryptography`, `Zero copy`, `Sharding algorithms`, `egui`, `Kotlin`, `Java`, `Spark`, `Flink`, `Airflow` and `Python`.**
